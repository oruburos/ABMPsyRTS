...
if __name__ == '__main__':
    ...
    br_pos =     br.get_agent_vars_dataframe()

    #print(br_pos.columns)

    # for i in range(len(br_pos["position"])):
    #     print(" Run {} it {} agent    pos {}".format(br_pos["Run"][i],  i,   br_pos["position"][i]))

   # br_pos.to_csv("PositionsAgents.csv")
    print(br_df["Data Collector"][0].get_model_vars_dataframe().columns)
    for i in range(len(br_df["Data Collector"])):
        if isinstance(br_df["Data Collector"][i], DataCollector):
            i_run_data = br_df["Data Collector"][i].get_model_vars_dataframe()
            print(i_run_data['Trajectories'])
